    ...
    <div className="flex h-full bg-surface overflow-hidden">
      ...
      <AnimatePresence>
        {intelligenceModal?.isOpen && (
          <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-2 sm:p-4 bg-black/60 backdrop-blur-md">
            <motion.div 
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className={`w-full ${!intelligenceModal.insight ? 'max-w-md' : 'max-w-[95vw] xl:max-w-4xl'} h-[calc(100dvh-1rem)] sm:h-auto max-h-[calc(100dvh-1rem)] sm:max-h-[90vh] bg-surface border border-outline/20 rounded-2xl sm:rounded-[32px] overflow-hidden shadow-2xl relative flex flex-col min-h-0 transition-all duration-300 ease-out`}
            >
              {/* Header */}
