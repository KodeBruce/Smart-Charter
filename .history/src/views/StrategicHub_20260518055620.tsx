...
export default function StrategicHub() {
  ...
  useEffect(() => {
    if (!isSentinelOpen) return;
    if (liveAgentFeed.length > 0) return;
    ...
    <div className="flex h-full bg-surface overflow-hidden">
      ...
      {!isSentinelOpen && (
        <button
          ...
          className="lg:hidden fixed right-0 top-1/2 -translate-y-1/2 z-50 h-14 w-8 rounded-l-xl border border-outline/20 border-r-0 bg-surface-container/95 backdrop-blur-sm shadow-lg flex items-center justify-center text-primary/70 hover:text-primary hover:bg-primary/10 transition-all"
        >
