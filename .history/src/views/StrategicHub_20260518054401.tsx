      ...
      {typeof window !== 'undefined' && createPortal(
        <AnimatePresence>
          {intelligenceModal?.isOpen && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4 bg-black/65 backdrop-blur-md">
              <motion.div 
                initial={{ opacity: 0, scale: 0.98, y: 12 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.98, y: 12 }}
                className={`w-[96vw] sm:w-full ${!intelligenceModal.insight ? 'max-w-[560px]' : 'max-w-[96vw] xl:max-w-4xl'} max-h-[88dvh] sm:max-h-[90vh] bg-surface border border-outline/20 rounded-2xl sm:rounded-[32px] overflow-hidden shadow-2xl relative flex flex-col min-h-0 transition-all duration-300 ease-out`}
              >
              {/* Header */}
                      ...
                      <div className="space-y-4 sm:space-y-5 min-h-0">
                          ...
                          <div className="flex items-center gap-2 min-w-max">
                            {tabs.map((tab) => (
                              <button
                                ...
                                className={`px-3 sm:px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border ${
                                  activeKey === tab.key
                        ...
                        <AnimatePresence mode="wait" initial={false}>
                          <motion.div
                            ...
                            className="min-h-[220px] max-h-[52dvh] sm:max-h-[56dvh] overflow-y-auto custom-scrollbar"
                          >
                            {activeKey === 'summary' && (
                        ...
                        <div className="flex items-center justify-between gap-2 pt-1">
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-wider bg-surface-container border border-outline/15 text-on-surface/60 disabled:opacity-40 disabled:cursor-not-allowed hover:text-on-surface hover:border-outline/30 transition-all"
                          >
                            Prev
                          </button>
                          <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-on-surface/35">Swipe or tap headings</span>
                          <button
                            onClick={() => goToIndex(activeIndex + 1)}
