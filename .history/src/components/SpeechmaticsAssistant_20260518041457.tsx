...
function speak(text: string, lang = 'en-US', preferredVoiceName?: string) {
  try {
    window.speechSynthesis.cancel();
    const utterance = new SpeechSynthesisUtterance(text);
    utterance.rate = 1.0; // slightly natural
    utterance.pitch = 1.0;
    ...
    try {
      ...
      if (preferredVoiceName) chosen = voices.find(v => v.name === preferredVoiceName);
      if (!chosen) {
        const stored = localStorage.getItem('preferredTTSVoice');
        if (stored) chosen = voices.find(v => v.name === stored);
      }

      // heuristics: prefer neural/Google/Wavenet voices, then match lang
      if (!chosen) {
        chosen = voices.find(v => /google|wave|neural|wavenet|premium|samantha|alloy|eleven/i.test(v.name));
      }
      if (!chosen && lang) {
...
export default function SpeechmaticsAssistant() {
  ...
  const [preferredVoiceName, setPreferredVoiceName] = useState<string | null>(() => {
    try { return localStorage.getItem('preferredTTSVoice'); } catch { return null; }
  });
  ...
  // Load available TTS voices and pick a sensible default
  useEffect(() => {
    const loadVoices = () => {
      try {
        const vs = window.speechSynthesis.getVoices() || [];
        setAvailableVoices(vs);
        if (!preferredVoiceName && vs.length) {
          const auto = vs.find(v => /google|wave|neural|wavenet|premium|samantha|alloy|eleven/i.test(v.name))
            || vs.find(v => v.lang && v.lang.startsWith(voiceLanguage))
            || vs[0];
          if (auto) {
            setPreferredVoiceName(auto.name);
            try { localStorage.setItem('preferredTTSVoice', auto.name); } catch {}
          }
        }
      } catch {}
    };
  ...
  }, [preferredVoiceName, voiceLanguage]);
                ...
                <div className="ml-2">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      try { if (v) localStorage.setItem('preferredTTSVoice', v); else localStorage.removeItem('preferredTTSVoice'); } catch {}
                    }}
                    className="bg-transparent text-[10px] text-white/70 px-2 py-1 rounded border border-white/5"
                    aria-label="Select TTS voice"
                  >
                    {availableVoices.filter(v => v.lang?.toLowerCase().startsWith(voiceLanguage)).map(v => (
