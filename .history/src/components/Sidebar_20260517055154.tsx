...
export default function Sidebar() {
  ...
  const mobileExtrasRef = useRef<HTMLDivElement | null>(null);
  ...
  // Close mobile extras when clicking outside
  useEffect(() => {
    const onDocClick = (e: MouseEvent) => {
      const target = e.target as Node | null;
      if (!mobileExtrasOpen) return;
      if (mobileExtrasRef.current && mobileExtrasRef.current.contains(target)) return;
      if (mobileNavRef.current && mobileNavRef.current.contains(target)) return;
  ...
  // Close mobile extras when navigation changes
  useEffect(() => {
    setMobileExtrasOpen(false);
  }, [location.pathname]);
  ...
  return (
    <>
      ...
      <nav ref={mobileNavRef} className="fixed bottom-4 left-1/2 -translate-x-1/2 md:hidden w-[92%] max-w-lg bg-surface/95 backdrop-blur rounded-full border border-outline px-2 py-2 flex flex-col items-center gap-2 z-50 shadow-lg overflow-visible">
        {/* Expandable extras (top) */}
        <motion.div
          ref={mobileExtrasRef}
          initial={{ scaleY: 0, opacity: 0 }}
          animate={mobileExtrasOpen ? { scaleY: 1, opacity: 1 } : { scaleY: 0, opacity: 0 }}
          transition={{ type: 'spring', stiffness: 400, damping: 28 }}
          style={{ transformOrigin: 'bottom center' }}
          className="w-full overflow-hidden rounded-full px-3"
          onClick={(e) => e.stopPropagation()}
          aria-hidden={!mobileExtrasOpen}
        >
          <div className="w-full flex items-center justify-around gap-3 bg-surface-container/80 backdrop-blur-md rounded-full py-2 shadow-md">
          <button
            onClick={toggleVoice}
            aria-label="Toggle voice"
            className={`p-2 rounded-md ${isListening ? 'text-secondary bg-primary/10' : 'text-on-surface-variant/60'}`}
          >
          ...
          <button
            ...
            className="p-2 rounded-md text-on-surface-variant/60"
          >
        ...
        {/* Chevron between extras and main row */}
        <div className="-mt-4 z-50 flex items-center justify-center">
          <button
            aria-label="Toggle quick actions"
            onClick={() => setMobileExtrasOpen(v => !v)}
            className="h-9 w-9 rounded-full bg-surface border border-outline flex items-center justify-center shadow-sm transform transition-transform"
            ...
            {navItems.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  to={item.path}
                  title={item.label}
                  className={`flex-1 flex items-center justify-center py-1 px-2 transition-colors text-[11px] ${isActive ? 'text-primary' : 'text-on-surface-variant/60 hover:text-on-surface'}`}
                >
