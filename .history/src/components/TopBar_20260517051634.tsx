...
export default function TopBar({ title, showSearch = true, actions }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-14 w-full items-center justify-between border-b border-outline bg-surface px-4 sm:px-6 lg:px-8">
      <div className="flex flex-1 items-center gap-6">
        ...
        {showSearch && (
          <button className="sm:hidden p-2 text-on-surface-variant/40 hover:text-primary" onClick={() => setMobileSearchOpen(v => !v)} aria-label="Search">
            <Search className="h-4 w-4" />
          </button>
        )}
        {mobileSearchOpen && (
          <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[92%] max-w-sm sm:hidden z-50">
            <div className="bg-surface-container-low border border-outline rounded-lg p-2 flex items-center">
              <Search className="h-4 w-4 text-on-surface-variant/40 mr-2" />
              <input autoFocus type="text" placeholder="Search files..." className="w-full bg-transparent outline-none text-[11px] font-semibold placeholder:text-on-surface-variant/30" />
              <button className="ml-2 text-on-surface-variant/40" onClick={() => setMobileSearchOpen(false)}>Close</button>
            </div>
          </div>
        )}
      </div>
