...
import { LayoutDashboard, FileText, Files, ShieldAlert, Settings, Plus, HelpCircle, Moon, Sun, Search, FolderOpen, Mic, MicOff, ChevronLeft, ChevronRight, ChevronUp } from 'lucide-react';
    ...
    <>
      ...
      {mobilePanelOpen && (
        <motion.div
          ...
          className="fixed left-0 right-0 bottom-0 md:hidden z-40 bg-surface border-t border-outline rounded-t-2xl p-3 shadow-2xl"
        >
          <div className="flex items-center justify-between mb-3">
            <div className="text-sm font-bold">Quick Menu</div>
            <button aria-label="Close quick menu" onClick={() => setMobilePanelOpen(false)} className="p-2 rounded-md text-on-surface-variant/60 hover:text-on-surface">
              <ChevronUp className="h-4 w-4 rotate-180" />
            </button>
          </div>
          <div className="grid grid-cols-4 gap-3">
            {navItems.map(item => (
              <NavLink key={`quick-${item.path}`} to={item.path} className="flex flex-col items-center gap-1 py-2">
                <item.icon className="h-5 w-5 text-on-surface/70" />
                <span className="text-[10px] text-on-surface/60">{item.label.split(' ')[0]}</span>
              </NavLink>
            ))}
            <button onClick={openIngest} className="flex flex-col items-center gap-1 py-2">
              <Plus className="h-5 w-5 text-secondary" />
              <span className="text-[10px] text-on-surface/60">Upload</span>
            </button>
          </div>
        </motion.div>
      )}
