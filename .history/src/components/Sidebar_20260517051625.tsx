...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 md:hidden w-[94%] max-w-lg bg-surface/95 backdrop-blur rounded-full border border-outline px-3 py-2 flex items-center justify-between gap-2 z-50 shadow-lg">
        {navItems.map((item) => {
          const isActive = location.pathname === item.path;
          return (
            <NavLink
              ...
              to={item.path}
              title={item.label}
              className={`flex-1 flex flex-col items-center justify-center gap-1 py-2 transition-colors text-[10px] ${isActive ? 'text-primary' : 'text-on-surface-variant/60 hover:text-on-surface'}`}
            >
              <item.icon className={`h-5 w-5 ${isActive ? 'stroke-[2.5]' : ''}`} />
              <span className="text-[9px] font-bold uppercase tracking-widest opacity-90">{item.label.split(' ')[0]}</span>
            </NavLink>
          );
        })}

        <button onClick={openIngest} className="-ml-2 mr-1 h-10 w-10 rounded-xl bg-secondary flex items-center justify-center text-secondary-content shadow-md">
