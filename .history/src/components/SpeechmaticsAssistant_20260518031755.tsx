...
export default function SpeechmaticsAssistant() {
  ...
  const startSession = async () => {
    ...
    // Pre-check authentication to avoid a quick "show-then-disappear" flash
    if (!auth.currentUser) {
      setError('Sign in to use voice commands.');
  ...
  return (
    <div className="fixed bottom-[calc(1.5rem+env(safe-area-inset-bottom))] sm:bottom-6 right-6 z-[60] pointer-events-none max-w-[92vw]">
      <AnimatePresence>
        {isListening && (
          <motion.div
      ...
      {!isListening && (
        <div
          ...
          role="button"
          tabIndex={0}
          className="pointer-events-auto inline-flex items-center gap-2 rounded-full border border-outline/30 bg-surface/90 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.22em] text-on-surface/50 shadow-lg shadow-black/5 cursor-pointer"
          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') toggleVoice(); }}
        >
          <MicOff className="h-3.5 w-3.5" />
          {error ? 'Voice error — sign in' : 'Speechmatics idle'}
        </div>
      )}
    </div>
  );
