      ...
      <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 md:hidden w-[92%] max-w-lg bg-surface/95 backdrop-blur rounded-full border border-outline px-2 py-1 flex items-center justify-between gap-2 z-50 shadow-lg">
        ...
        {mobileExtrasOpen && (
          <motion.div
            ...
            exit={{ y: 20, opacity: 0 }}
            className="absolute -bottom-20 left-1/2 -translate-x-1/2 z-40 w-[86%] bg-surface-container border border-outline rounded-2xl p-2 flex items-center justify-around gap-3 shadow-lg"
          >
            <button
              onClick={toggleVoice}
