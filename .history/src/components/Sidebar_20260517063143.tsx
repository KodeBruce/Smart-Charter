...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <motion.nav
        animate={{ height: mobileExtrasOpen ? 126 : 56 }}
        transition={{ type: 'spring', stiffness: 340, damping: 30 }}
        className="fixed bottom-4 left-1/2 -translate-x-1/2 md:hidden w-[92%] max-w-lg bg-surface/95 backdrop-blur rounded-[28px] px-2 py-1 z-50 shadow-lg overflow-hidden"
      >
        <div className="relative h-full w-full">
          <div className="absolute inset-x-0 bottom-0 flex items-center gap-2 px-1 pb-1">
            <button
          ...
          <motion.div
            ...
            className="absolute inset-x-1 bottom-12 grid grid-cols-3 gap-3 rounded-2xl border border-outline bg-surface-container px-3 py-2"
          >
            <button
              onClick={toggleVoice}
