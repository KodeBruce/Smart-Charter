...
function speak(text: string, lang = 'en-US') {
  try {
    // Select a natural-sounding voice when available. Cache per-language selection.
    const getVoices = () => {
    ...
    const preferredKeywords = ['Google', 'Neural', 'Wavenet', 'WaveNet', 'Microsoft', 'Amazon', 'Samantha', 'Joanna', 'Matthew', 'Alloy', 'Aria', 'Emma', 'Olivia', 'Ivy'];
    const forbidden = ['robot', 'synth', 'io'];
    const pickVoice = async (langTag: string) => {
      ...
      for (const v of voices) {
        ...
        try {
          ...
          if (vlang.startsWith(langPrefix)) score += 30;
          // prefer provider/natural names
          for (const k of preferredKeywords) if ((v.name || '').includes(k)) score += 40;
          // penalize clearly synthetic/robotic names
          for (const f of forbidden) if (name.includes(f)) score -= 100;
        } catch {}
        if (score > bestScore) {
          bestScore = score;
          best = v;
    ...
    if (!chosen) {
      // pick asynchronously but speak will wait for selection
      // this keeps behavior deterministic without exposing voice choices
      // to the user.
      // eslint-disable-next-line @typescript-eslint/no-floating-promises
      (async () => {
        const v = await pickVoice(lang);
        try { (window as any)[cachedKey] = v; } catch {}
      })();
    }

    // Use synchronous voices list if available immediately
    const voicesNow = window.speechSynthesis.getVoices();
    if (voicesNow && voicesNow.length) {
      chosen = chosen || voicesNow.find(v => v.lang && v.lang.startsWith((lang || 'en').split('-')[0])) || voicesNow[0];
    }
