...
export default function VoiceController() {
  ...
  const speakText = (text: string) => {
    ...
    // Choose high-quality US natural voice
    ...
    // Set speaking flag to true before stopping
    ...
    const handleSpeechEnd = () => {
      ...
      if (isListening) {
        try {
          ...
          console.warn('Speech restart blocked or already running:', err);
        }
      }
    };
      ...
      <AnimatePresence>
        {isListening && (
          <motion.div 
            ...
            className="fixed bottom-6 right-6 z-[60] w-[350px] bg-[#0D0D0E]/95 backdrop-blur-2xl border border-white/10 p-5 rounded-[28px] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] flex flex-col gap-4 overflow-hidden text-white"
          >
            ...
            <div className="pt-3 border-t border-white/10 flex items-center justify-between">
              ...
              <button 
                ...
                className="px-4 py-1.5 border border-white/10 text-white/60 hover:text-white text-[8px] font-black uppercase tracking-widest rounded-xl hover:bg-white/10 transition-all"
              >
                Close Mic
              </button>
            </div>
